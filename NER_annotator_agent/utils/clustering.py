...
def main():
    ...
    parser.add_argument("--input", type=str, default="/home/tiziano/langgraph_agents/NER_annotator_agent/data/output/gemini_order_ner_dataset.jsonl", help="Path to input .jsonl file")
    parser.add_argument("--output", type=str, default="/home/tiziano/langgraph_agents/NER_annotator_agent/data/output/order_rag_db.json", help="Path to output .jsonl file")
    ...
    if args.auto_k:
        ...
        while True:
            try:
                k_input = input(f"\nEnter number of clusters (suggested: {suggested_k}, range: 2-{args.max_k}): ")
    ...
    results = retrieve_top_k_per_centroid(embeddings, entries, labels, centroids, args.top_k)
    
    # Salva risultati
    save_output(results, args.output)
    
    print(f"[DONE] Successfully processed {len(entries)} documents into {k_chosen} clusters")
    print(f"[DONE] Saved {len(results)} top-k documents to {args.output}")
